...
static void testHv8();
static void testHv9();
static void testHv10();
static void testHv11();
static void testHv12();
...
static void testHv7()
{
    ...
    assert(HvGetScore(hv) == GAME_START_SCORE - 4 * SCORE_LOSS_DRACULA_TURN - SCORE_LOSS_HUNTER_HOSPITAL);
...
static void testHv10()
{
    ...
    assert(HvGetScore(hv) == GAME_START_SCORE - 7 * SCORE_LOSS_DRACULA_TURN - SCORE_LOSS_VAMPIRE_MATURES);
    assert(HvGetPlayerLocation(hv, PLAYER_DRACULA) == CITY_UNKNOWN);
    assert(HvGetVampireLocation(hv) == NOWHERE);
